...
<article x-data="{ open: false }" x-init="$watch('open', value => { if (value) { document.body.classList.add('overflow-hidden') } else { document.body.classList.remove('overflow-hidden') } })"
    class="fade-in-up card w-full bg-base-100 shadow-lg rounded-xl border border-base-200 hover:shadow-xl transition-all duration-300 overflow-hidden group"
    itemscope itemtype="https://schema.org/Product" @keydown.escape.window="open = false">
    ...
    <div class="flex items-center gap-3 p-3 relative">
        ...
        @if ($product->getFirstMediaUrl())
            <figure @click="open = true"
                ...
                <img loading="lazy" src="{{ $product->getFirstMediaUrl() }}"
                    alt="Image of {{ $product->name }} – {{ $product->description }}"
        ...
        <div class="flex-1 min-w-0">
            <header class="flex justify-between items-start mb-1">
                <h3 class="font-bold text-base truncate flex space-x-1 items-center justify-center" itemprop="name">
                    @if ($product->featured)
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                            class="size-5 text-yellow-500" aria-label="Featured product">
                    ...
                    @endif
                    @if ($product->new)
                        <img src="{{ asset('icons/new.png') }}" alt="new" class="w-6 mx-2">
                    @endif
    ...
    <div x-show="open" @click="open = false"
        class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50" x-transition.opacity>
    ...
    @php
        ...
        if ($product->getFirstMediaUrl()) {
            $jsonData['image'] = $product->getFirstMediaUrl();
        }
    @endphp

    <script type="application/ld+json">
    {!! json_encode($jsonData, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) !!}
    </script>
